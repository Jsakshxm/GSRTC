...
const page = () => {
    ...
    const router=useRouter;
        ...
        const handleSubmit = async (e) => {
            e.preventDefault()
            if (!Email || !Password) {
              SetError("Please fill the full form");
              return;
            }
...
let { data, error } = await supabase.auth.signInWithPassword({
    ...
    password: Password
              ...
              if (error) {
                ...
                SetError(null);
                console.log("User Login up:", user);
                ()=>{
                  router.push("/Bus")
                }
  ...
  return (
    <div>
      ...
      <form className="form">
        ...
        <input className="input" name="email" placeholder="Email" type="email"  value={Email}/>
        <input className="input" name="password" placeholder="Password" type="password" value={Password} />
        ...
        <button className="button-confirm">Let`s go →</button> <Link href="PassengerSignup"><p className='p-2 m-2 text-lg font-semibold'>Don't have an Account?</p> </Link> </form> 
